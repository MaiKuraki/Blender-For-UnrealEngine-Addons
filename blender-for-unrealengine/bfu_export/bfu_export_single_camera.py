...
def process_camera_export_from_asset(
    op: bpy.types.Operator,
    asset: AssetToExport,
    pre_bake_camera: 'bfu_camera.bfu_camera_data.BFU_CameraTracks' = None
) -> bfu_export_logs.bfu_asset_export_logs.ExportedAssetLog:

    obj = asset.obj
    my_asset_log = process_camera_export(op, obj, pre_bake_camera)
    my_asset_log.unreal_target_import_path = asset.import_dirpath
    return my_asset_log

def process_camera_export(
    op: bpy.types.Operator,
    ...
    if fullpath:
        ...
        if obj.bfu_export_fbx_camera:
            ...
            file.file_type = "FBX"
            export_single_fbx_camera(op, fullpath, obj)
